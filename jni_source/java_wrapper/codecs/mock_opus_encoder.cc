...
int GetFrameSizeMs(const SdpAudioFormat& format) {
	...
	return MockOpusEncoder::Config::kDefaultFrameSizeMs;
}
...
rtc::Optional<AudioCodecInfo> MockOpusEncoder::QueryAudioEncoder(
...
MockOpusEncoder::Config MockOpusEncoder::CreateConfig(
		const CodecInst& codec_inst) {
	MockOpusEncoder::Config config;
	config.frame_size_ms = rtc::CheckedDivExact(codec_inst.pacsize, 48);
	config.num_channels = codec_inst.channels;
	config.bitrate_bps = rtc::Optional<int>(codec_inst.rate);
	config.payload_type = codec_inst.pltype;
	config.application = config.num_channels == 1 ? MockOpusEncoder::kVoip
			: MockOpusEncoder::kAudio;
...
class MockOpusEncoder::PacketLossFractionSmoother {
public:
...
MockOpusEncoder::Config::Config(const Config&) = default;
MockOpusEncoder::Config::~Config() = default;
auto MockOpusEncoder::Config::operator=(const Config&) -> Config& = default;

bool MockOpusEncoder::Config::IsOk() const {
...
int MockOpusEncoder::Config::GetBitrateBps() const {
	RTC_DCHECK(IsOk());
	if (bitrate_bps)
...
rtc::Optional<int> MockOpusEncoder::Config::GetNewComplexity() const {
...
MockOpusEncoder::MockOpusEncoder(
		const Config& config,
...
MockOpusEncoder::MockOpusEncoder(const CodecInst& codec_inst)
: MockOpusEncoder(CreateConfig(codec_inst), nullptr) {
}

MockOpusEncoder::MockOpusEncoder(int payload_type,
		const SdpAudioFormat& format)
: MockOpusEncoder(CreateConfig(payload_type, format), nullptr) {
}
...
int MockOpusEncoder::SampleRateHz() const {
	return kSampleRateHz;
}

size_t MockOpusEncoder::NumChannels() const {
	return config_.num_channels;
}

size_t MockOpusEncoder::Num10MsFramesInNextPacket() const {
	return Num10msFramesPerPacket();
}

size_t MockOpusEncoder::Max10MsFramesInAPacket() const {
	return Num10msFramesPerPacket();
}

int MockOpusEncoder::GetTargetBitrate() const {
	return config_.GetBitrateBps();
}

void MockOpusEncoder::Reset() {
	RTC_CHECK(RecreateEncoderInstance(config_));
}

bool MockOpusEncoder::SetFec(bool enable) {
/*
	if (enable) {
...
*/
	if (!enable) {
		...
		config_.fec_enabled = false;
		return false;
	}

}

bool MockOpusEncoder::SetDtx(bool enable) {
/*
	if (enable) {
...
bool MockOpusEncoder::GetDtx() const {
	return config_.dtx_enabled;
}

bool MockOpusEncoder::SetApplication(Application application) {
	auto conf = config_;
	switch (application) {
	case Application::kSpeech:
		conf.application = MockOpusEncoder::kVoip;
		break;
	case Application::kAudio:
		conf.application = MockOpusEncoder::kAudio;
		break;
	}
	return RecreateEncoderInstance(conf);
}

void MockOpusEncoder::SetMaxPlaybackRate(int frequency_hz) {
	auto conf = config_;
	conf.max_playback_rate_hz = frequency_hz;
	RTC_CHECK(RecreateEncoderInstance(conf));
}

bool MockOpusEncoder::EnableAudioNetworkAdaptor(
...
void MockOpusEncoder::DisableAudioNetworkAdaptor() {
	audio_network_adaptor_.reset(nullptr);
}

void MockOpusEncoder::OnReceivedUplinkPacketLossFraction(
...
void MockOpusEncoder::OnReceivedUplinkRecoverablePacketLossFraction(
...
void MockOpusEncoder::OnReceivedUplinkBandwidth(
	...
	if (audio_network_adaptor_) {
		...
		if (!overhead_bytes_per_packet_) {
			LOG(LS_INFO)
        				  << "MockOpusEncoder: Overhead unknown, target audio bitrate "
...
void MockOpusEncoder::OnReceivedRtt(int rtt_ms) {
	if (!audio_network_adaptor_)
		return;
...
void MockOpusEncoder::OnReceivedOverhead(size_t overhead_bytes_per_packet) {
	if (audio_network_adaptor_) {
...
void MockOpusEncoder::SetReceiverFrameLengthRange(int min_frame_length_ms,
		int max_frame_length_ms) {
...
AudioEncoder::EncodedInfo MockOpusEncoder::EncodeImpl(
...
size_t MockOpusEncoder::Num10msFramesPerPacket() const {
	return static_cast<size_t>(rtc::CheckedDivExact(config_.frame_size_ms, 10));
}

size_t MockOpusEncoder::SamplesPer10msFrame() const {
	return rtc::CheckedDivExact(kSampleRateHz, 100) * config_.num_channels;
}

size_t MockOpusEncoder::SufficientOutputBufferSize() const {
...
// false.
bool MockOpusEncoder::RecreateEncoderInstance(const Config& config) {
	if (!config.IsOk())
		return false;
	config_ = config;
//	if (inst_)
//		RTC_CHECK_EQ(0, WebRtcOpus_EncoderFree(inst_));
	...
	std::cerr << " opus recreate num channels -> " << config.num_channels << std::endl;
//	RTC_CHECK_EQ(0, WebRtcOpus_EncoderCreate(&inst_, config.num_channels,
//			config.application));

	std::cerr << " opus recreate get bitrate -> " << config.GetBitrateBps() << std::endl;
...
//		RTC_CHECK_EQ(0, WebRtcOpus_DisableFec(inst_));
	...
	std::cerr << " opus recreate max playrate hz -> " << config.max_playback_rate_hz << std::endl;
//	RTC_CHECK_EQ(
//			0, WebRtcOpus_SetMaxPlaybackRate(inst_, config.max_playback_rate_hz));
	...
	std::cerr << " opus recreate complexity -> " << complexity_ << std::endl;
...
//		RTC_CHECK_EQ(0, WebRtcOpus_DisableCbr(inst_));
	...
	std::cerr << " opus recreate frame size ms-> " << config_.frame_size_ms << std::endl;
	return true;
}

void MockOpusEncoder::SetFrameLength(int frame_length_ms) {
	next_frame_length_ms_ = frame_length_ms;
}

void MockOpusEncoder::SetNumChannelsToEncode(size_t num_channels_to_encode) {
...
void MockOpusEncoder::SetProjectedPacketLossRate(float fraction) {
...
void MockOpusEncoder::SetTargetBitrate(int bits_per_second) {
...
void MockOpusEncoder::ApplyAudioNetworkAdaptor() {
...
MockOpusEncoder::DefaultAudioNetworkAdaptorCreator(
...
void MockOpusEncoder::MaybeUpdateUplinkBandwidth() {
	if (audio_network_adaptor_) {
		int64_t now_ms = rtc::TimeMillis();
