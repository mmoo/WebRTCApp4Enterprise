...
class MockOpusEncoder final : public AudioEncoder {
 public:
  ...
  explicit MockOpusEncoder(const CodecInst& codec_inst);
  MockOpusEncoder(int payload_type, const SdpAudioFormat& format);
  ~MockOpusEncoder() override;
