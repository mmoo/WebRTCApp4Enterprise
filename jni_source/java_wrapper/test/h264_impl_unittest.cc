...
#include "../codecs/mock_h264_encoder.h"



namespace webrtc {

class TestMockH264Impl : public VideoCodecTest {
 protected:
  VideoEncoder* CreateEncoder() override {
    return antmedia::MockH264Encoder::Create(cricket::VideoCodec(cricket::kH264CodecName));
...
TEST_F(TestMockH264Impl, MAYBE_EncodeDecode) {
  ...
  //we comment out the below check because decoded frame is not decoded from the encoded input frame
  //EXPECT_GT(I420PSNR(input_frame_.get(), decoded_frame.get()), 36);
}


TEST_F(TestMockH264Impl, MAYBE_DecodedQpEqualsEncodedQp) {
  EXPECT_EQ(WEBRTC_VIDEO_CODEC_OK,
            encoder_->Encode(*input_frame_, nullptr, nullptr));
  EncodedImage encoded_frame;
  CodecSpecificInfo codec_specific_info;
