...
class JavaAudioSinkWrapper
	...
	virtual void OnData(const void* audio_data,
		...
		int bytePerSample = bits_per_sample / 8;
		int size = bytePerSample * number_of_channels * number_of_frames;
		jbyteArray byteArray = jni()->NewByteArray(size);

		jni()->SetByteArrayRegion(byteArray, 0, size, (jbyte*)audio_data);

		jni()->CallVoidMethod(*j_callbacks_, j_sink_frame_id_, byteArray, bits_per_sample,sample_rate,number_of_channels,number_of_frames);
		CHECK_EXCEPTION(jni());
	...
	jobject CricketToJavaI420Frame(const webrtc::VideoFrame* frame) {
		...
		return jni()->NewObject(
				*j_frame_class_, j_i420_frame_ctor_id_,
				...
				strides, planes, javaShallowCopy(frame), frame->timestamp_us());
	}
...
JOW(void, AudioSink_freeWrappedAudioSink)(JNIEnv*, jclass, jlong j_p) {
	delete reinterpret_cast<JavaAudioSinkWrapper*>(j_p);
}
...
: network_thread_(std::move(network_thread)),
  ...
  audio_encoder_factory_(audio_encoder_factory){}
	...
	AudioDeviceModule* audio_device_module() { return cadm_.get(); };

	void clear_network_monitor_factory() { network_monitor_factory_ = nullptr; }
	void InvokeJavaCallbacksOnFactoryThreads();
	void setVideoCapturer(CustomVideoCapturer* videoCapturer) {
		videoCapturer_ = videoCapturer;
	}
...
PeerConnectionFactoryInterface::Options ParseOptionsFromJava(JNIEnv* jni,
	...
	bool disable_encryption =
			(jni->GetBooleanField(options, disable_encryption_field) == JNI_TRUE);

	jfieldID disable_network_monitor_field =
			jni->GetFieldID(options_class, "disableNetworkMonitor", "Z");
	bool disable_network_monitor =
			jni->GetBooleanField(options, disable_network_monitor_field) == JNI_TRUE;
...
JOW(jlong, PeerConnectionFactory_nativeCreatePeerConnectionFactory)(
	...
	worker_thread->Invoke<void>(RTC_FROM_HERE, [&] {
		//TODO: we may not need to run this in another thread
		adm = CustomAudioDeviceModule::Create(0, CustomAudioDeviceModule::AudioLayer::kLinuxAlsaAudio);
		//LOG(WARNING)<< "after creating adm" << adm.release();
	});
...
JOW(jboolean, PeerConnectionFactory_nativeStartAecDump)(
	...
	rtc::scoped_refptr<PeerConnectionFactoryInterface> factory(
			factoryFromJava(native_factory));
	return factory->StartAecDump(reinterpret_cast<rtc::PlatformFile>(file), filesize_limit_bytes);
}
...
JOW(void, PeerConnectionFactory_nativeSetVideoHwAccelerationOptions)(
	...
	//		static_cast<WebRtcVideoEncoderFactory*>
...
JOW(bool, PeerConnection_nativeStartRtcEventLog)(
	...
	return ExtractNativePC(jni, j_pc)->StartRtcEventLog(reinterpret_cast<rtc::PlatformFile>(file_descriptor),
			max_size_bytes);
}
...
JOW(jlong, AudioSink_nativeWrapAudioSink)(
		JNIEnv* jni, jclass, jobject j_callbacks) {
	std::unique_ptr<JavaAudioSinkWrapper> renderer(
			new JavaAudioSinkWrapper(jni, j_callbacks));
	return (jlong)renderer.release();
}
